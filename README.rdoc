...
* Optionally defina a +crud_attrs+ class variable that is an array or block that returns an array of model field names in your controller to limit what is crud'able.
...
* To make customizing the show pages easier with bootstrap 5 tabs, there is a TabsHelper module - see the comments in there for usage.
...
* This fork DOES have dependencies - pagy for pagination, pundit for authorization, and slim for views.  They were added because we always (so far) use these features.  In the future, we could abstract them out and only use them if they're present or using some kind of configration with an adapter pattern.
...
If you find yourself adapting the same parts of dry_crud for your applications over and over (we did), please feel free to {fork me on Github}[http://github.com/codez/dry_crud].
...
For :string attributes on your model, by default, a text_field will be rendered.  If you want a dropdown select field, define a specially named helper method with the select choices.  Name the method select_choices_<model_name>_<field_name> or select_choices_<field_name>. The method will be passed the model object as its only parameter.  It should return an array or hash that can be used by the form.select method to create the options.
